...
class BeautifulReport(ReportTestResult, PATH):
    ...
    def report(self, description, filename: str = None, report_dir=os.path.split(os.path.realpath(__file__))[0],
        ...
        my_dir = report_dir
        os.makedirs(report_dir, 0o775, exist_ok=True)
        self.suites.run(result=self)
        self.stopTestRun(self.title)
        self.output_report(theme)
        text = '\n测试已全部完成, 可打开 {} 查看报告'.format(os.path.join(my_dir, self.filename))
        print(text)
    ...
    def output_report(self, theme):
        ...
        with open(template_path, 'rb') as file:
            ...
            file = report_path + '/' + self.filename

        file_test = open(file, 'w', encoding='utf-8')
        html = render_template(render_params, body)
        file_test.write(html)
        file_test.close()
