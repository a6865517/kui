...
"""
import requests
from utils import mysql_utils_test,Log
from common import sqls, test_data, test_urls
...
def get_gold_device_sno():
    ...
    data = mysql_utils_test.cursor_sql(sql)
    return data[0]['sno']
...
def get_gold_device_id(sno):
    ...
    data = mysql_utils_test.cursor_sql(sql, sno)
...
def bind_gold_blood_device(clinic_id, device_id, login_headers) -> None:
    ...
    :return:
    """
    bind_device_data = test_data.bind_device_data
    bind_device_url = test_urls.bind_device_url
...
def get_clinic_bind_device_sno_gold(clinic_id):
    ...
    sno = mysql_utils_test.cursor_sql(sql, clinic_id)
    return sno[0]['sno']
