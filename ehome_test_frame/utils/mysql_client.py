# -*- encoding: utf-8 -*-
"""
@File    : mysql-client.py
@Time    : 2022/7/16 14:55
@Author  : kui
@Software: PyCharm
"""
import pymysql
...
class MysqlClient(object):
    ...
    def __init__(self, db='business'):
        ...
        ENV = env.env()
        try:
            connect_config = read_yaml.read(f'../config/{ENV}/{ENV}.yaml').get("mysql")
        except AttributeError as e:
            print('config文件mysql配置项有误',e)
        mysql_config = connect_config.get(db)
        mincached = 10
        maxcached = 20
        maxshared = 10
        maxconnections = 100
